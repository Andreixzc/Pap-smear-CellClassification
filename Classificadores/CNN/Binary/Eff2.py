...
import numpy as np
import time  # Importe o módulo time para medir o tempo
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        label = 0 if label == 'Negative' else 1  # Converte para 0 se 'Negative', caso contrário, 1

        if self.transform:
            image = self.transform(image)

        label = torch.tensor(label).float()  # Converte o label para tensor de ponto flutuante
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs=20, save_path='Effnet_Binary_Weights.pth'):
    ...
    for epoch in range(num_epochs):
        start_time = time.time()  # Inicia a contagem do tempo no início da época
        model.train()
        running_loss = 0.0
        correct_train = 0
        total_train = 0

        # Utilize tqdm para mostrar o progresso do treinamento
        ...
        elapsed_time = time.time() - start_time  # Calcule o tempo decorrido para a época
        print(f'Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss:.4f}, Train Acc: {epoch_acc_train:.4f}, Test Acc: {epoch_acc_test:.4f}, Time: {elapsed_time:.2f} seconds')
